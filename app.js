...
window.borrarCliente = function(index) {
    if(confirm(`¿Seguro que quieres borrar a este cliente y todas sus obras?`)) {
...
window.renderPresupuesto = function() {
    ...
    document.getElementById('desglose-precios').innerHTML = `
        ...
        ${trabajoActual.lineas.map(l => `<div class="border-b py-2 uppercase text-[11px]"><div class="flex justify-between font-black"><span>${l.icono} ${l.nombre}</span><span>${(l.cantidad*l.precio).toFixed(2)}€</span></div><div class="text-[9px] text-slate-400 font-bold">${l.cantidad.toFixed(2)} ${CONFIG[l.tipo].uni} x ${l.precio}€</div></div>`).join('')}
        <button onclick="abrirFirma()" class="w-full mt-4 border-2 border-dashed border-blue-200 py-3 rounded-xl text-blue-500 font-bold text-[10px] uppercase italic">Añadir Firma Cliente</button>
...
window.generarPDF = function(i) {
    ...
    el.innerHTML = `<h2 style="color:#2563eb">PRESUPUESTO: ${p.lugar}</h2><p><strong>Cliente:</strong> ${clienteActual.nombre}</p><hr>
